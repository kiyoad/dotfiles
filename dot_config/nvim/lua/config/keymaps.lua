...
-- skkeleton
vim.keymap.set({ "i", "c" }, "<C-o>", [[<Plug>(skkeleton-toggle)]], { noremap = false })
--[[
 SKK keybindings
 (insert mode)CTRL-o : 全角ひらがな入力
 (Insert mode(SKK))q : 全角カタカナ<->全角ひらがな
 (Insert mode(SKK))CTRL-q : 半角カタカナ<->全角ひらがな
 (Insert mode(SKK))L : 全角英数入力(CTRL-o で戻る)
 (Insert mode(SKK))l : 半角英数入力(insert mode)に戻る
 (Insert mode(SKK:SHIFTで入力開始))SPACE : 変換
 (Insert mode(SKK:SHIFTで入力開始))CTRL-o : 確定
